...
export default function AboutPage() {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-slate-950 transition-colors duration-500 selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black">
      <Navbar />
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center max-w-5xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <h2 className="text-3xl md:text-4xl font-bold mb-6 text-black dark:text-white tracking-tight">{profile?.name || "Hikra"}</h2>
              <div className="text-gray-600 dark:text-gray-400 leading-relaxed mb-6 space-y-4 whitespace-pre-wrap">
                {bio}
              </div>
            </motion.div>
          </div>
      ...
      <section className="py-20 bg-gray-50 dark:bg-slate-900 border-y border-gray-100 dark:border-slate-800">
        <div className="container mx-auto px-6 max-w-5xl">
          <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-10 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {Object.entries(groupedSkills).map(([category, catSkills], idx) => (
              <motion.div key={category} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}>
                <h4 className="font-bold text-sm uppercase tracking-wider mb-4 border-b border-gray-200 dark:border-slate-800 text-gray-900 dark:text-white pb-2">{category}</h4>
                <div className="flex flex-wrap gap-2">
                  {catSkills.map(skill => (
                    <span key={skill.id} className="px-3 py-1.5 text-[11px] font-medium border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-full text-gray-700 dark:text-gray-300">
      ...
      <section className="py-24 bg-white dark:bg-slate-950">
        <div className="container mx-auto px-6 max-w-4xl">
          ...
          <div className="space-y-12 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 dark:before:via-slate-800 before:to-transparent">
            {experiences.map((exp, idx) => (
              <motion.div key={exp.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-white dark:border-slate-950 bg-black dark:bg-white text-white dark:text-black shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow">
                  <span className="w-2 h-2 rounded-full bg-white dark:bg-black"></span>
                </div>
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_4px_25px_-5px_rgba(0,0,0,0.1)] dark:shadow-none transition-shadow">
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 gap-2">
                    <h4 className="font-bold text-lg text-black dark:text-white">
                      {language === "en" && exp.role_en ? exp.role_en : exp.role}
                    </h4>
                    <span className="text-xs font-bold px-3 py-1 bg-gray-100 dark:bg-slate-800 rounded-full text-gray-600 dark:text-gray-300 shrink-0">{exp.start_date} - {exp.end_date}</span>
                  </div>
                  <h5 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">
                    {language === "en" && exp.client_en ? exp.client_en : exp.client}
                  </h5>
                  <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-2 list-none">
                    {(language === "en" && exp.description_en ? exp.description_en : exp.description).split('\n').filter((line: string) => line.trim() !== '').map((line: string, i: number) => (
                      <li key={i} className="flex gap-2">
                        <span className="text-gray-300 dark:text-slate-600 mt-1.5">•</span>
                        <span>{line}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="space-y-6">
                {awards.filter(a => a.type === "exhibition").map((item) => (
                  <div key={item.id} className="flex gap-4">
                    <div className="w-12 text-xs font-bold text-gray-400 dark:text-gray-500 pt-1">{item.year}</div>
                    <div>
        ...
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(groupedGear).map(([category, items], idx) => (
              <motion.div key={category} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="p-6 border border-gray-200 dark:border-slate-800 rounded-2xl bg-gray-50/50 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-800 hover:shadow-lg dark:hover:shadow-none transition-all">
                <h4 className="font-bold uppercase tracking-wider text-xs text-gray-400 dark:text-gray-500 mb-4">{category}</h4>
                <ul className="space-y-3">
                  {items.map(item => (
                    <li key={item.id} className="font-medium text-sm text-black dark:text-gray-200 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 bg-black dark:bg-gray-400 rounded-full" />
